...
function gxRequireAuth(){if(location.pathname.endsWith('/login.html')||location.pathname.endsWith('login.html'))return;const s=gxGetSession();if(!s){const next=gxRememberReturnTo();location.replace(`login.html${next?`?next=${encodeURIComponent(next)}`:''}`);return}window.GX_CURRENT_USER=s}
...
const GX_PAGE_PERMISSION_MAP={
'index.html':'home','touchpoint.html':'services','standar.html':'services','map.html':'services','layanan.html':'services',
'pre-journey.html':'pre-journey','pre-flight.html':'pre-flight','post-flight.html':'post-flight','post-journey.html':'post-journey','inisiatif.html':'initiatives','calendar.html':'calendar',
...
'data.html':'data','berita.html':'news','kontak.html':'contact','admin.html':'admin','audit-log.html':'admin'
};
...
if(!location.pathname.endsWith('login.html')){gxRequireAuth();gxEnforcePageAccess()}
